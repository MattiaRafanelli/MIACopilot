...
public class DataService
{
    ...
    /// <summary>Loads all apprentices (including journals) from JSON.</summary>
    public List<Apprentice> LoadApprentices()
    {
        if (!File.Exists(ApprenticesFile)) return new List<Apprentice>();
        var json = File.ReadAllText(ApprenticesFile);
        return JsonSerializer.Deserialize<List<Apprentice>>(json, _options) ?? new();
    }

    /// <summary>Saves all apprentices (including journals) to JSON.</summary>
    ...
    /// <summary>Loads all companies from JSON.</summary>
    public List<Company> LoadCompanies()
    {
        if (!File.Exists(CompaniesFile)) return new List<Company>();
        var json = File.ReadAllText(CompaniesFile);
        return JsonSerializer.Deserialize<List<Company>>(json, _options) ?? new();
    }

    /// <summary>Saves all companies to JSON.</summary>
    ...
    /// <summary>Loads all vocational trainers from JSON.</summary>
    public List<VocationalTrainer> LoadTrainers()
    {
        if (!File.Exists(TrainersFile)) return new List<VocationalTrainer>();
        var json = File.ReadAllText(TrainersFile);
        return JsonSerializer.Deserialize<List<VocationalTrainer>>(json, _options) ?? new();
    }

    /// <summary>Saves all vocational trainers to JSON.</summary>

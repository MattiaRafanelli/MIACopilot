...
/// <summary>
/// Provides CRUD operations, averages and filtering for grades.
/// </summary>
public class GradeService
{
    ...
    /// <summary>Loads grades from JSON file.</summary>
    ...
    /// <summary>Saves all grades to JSON file.</summary>
    private void Save() =>
        File.WriteAllText(_file, JsonSerializer.Serialize(_grades, _options));

    // ── CRUD ─────────────────────────────────────────────────────────────────

    /// <summary>Returns all grades.</summary>
    public List<Grade> GetAll() => _grades;

    /// <summary>Returns all grades for a specific apprentice.</summary>
    public List<Grade> GetByApprentice(int apprenticeId) =>
        _grades.Where(g => g.ApprenticeId == apprenticeId).ToList();

    /// <summary>Finds a grade by ID.</summary>
    public Grade? GetById(int id) =>
        _grades.FirstOrDefault(g => g.Id == id);

    /// <summary>Creates a new grade and persists data.</summary>
    public void Create(Grade grade)
    {
        grade.Id = _grades.Any() ? _grades.Max(g => g.Id) + 1 : 1;
        _grades.Add(grade);
        Save();
    }

    /// <summary>Updates an existing grade and persists data.</summary>
    ...
    /// <summary>Deletes a grade by ID and persists data.</summary>
    ...
    /// <summary>Calculates the overall average for one apprentice.</summary>
    public double GetAverage(int apprenticeId)
    {
        var list = GetByApprentice(apprenticeId);
        return list.Any() ? Math.Round(list.Average(g => g.Value), 2) : 0;
    }

    /// <summary>Calculates the average per subject for one apprentice.</summary>
    ...
    /// <summary>Returns all grades for an apprentice filtered by subject.</summary>
    ...
    /// <summary>Returns all distinct subjects for one apprentice.</summary>
